...
export class MamWriter {
     ...
     */
    public async createAndAttach(message: string, tag?: string) {
        let Result: { payload: string, root: string, address: string } = this.create(message);

        let Result2 = await this.attach(Result.payload, Result.address, undefined, undefined, tag);
        return Result2;
    }
     ...
     */
    public async attach(payload: string, address: string, depth: number = 6, mwm: number = 14, tag: string = undefined): Promise<Transaction[]> {
